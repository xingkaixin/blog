...
上一篇我写了 [SDD 的问题](/posts/spec-as-new-fig-leaf/)：spec 写得足够细，本质上就是在用另一种语法写代码，它没有消灭工程的复杂度，只是换了个地方藏起来。
...
**前置依赖和约束。** 必须兼容的现有接口、不能动的模块、技术栈限制、性能要求。这些是硬性边界，AI 在边界内自由发挥，但不能突破。还要检查那些[你以为不用写的隐含假设](/posts/spec-unwritten-assumptions/)，避免不同模块各自采用不兼容的默认值。
