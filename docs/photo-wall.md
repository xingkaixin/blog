...
- 新上传的图片在月份记录中保存 `mediaRevision`，每次重新发布使用新版本路径，同一次发布的提交重试复用已上传版本。
...
扫描前先确认没有遗留的暂停发布进程；扫描后不要恢复旧发布进程，应重新运行发布命令生成新版本。
...
新回收记录的 `deleteAfter` 为 `null`，确认公开索引已更新后才写入回收时间。v1、v2 控制文档中的
待回收记录会重新确认公开索引并等待完整宽限期；v3 记录保留原有回收时间和领取状态。升级可能延后旧版本回收，但不会搬迁图片。
